...
def build(spark: SparkSession, cfg: dict, as_of_date: str) -> DataFrame:
    """How much of portfolio corporate-bond AUM finances issuer GHG emissions?"""
    with timed_step(LOGGER, "report.R02") as timer:
        ...
        filtered = fe.filter(
            (F.col("as_of_date") == F.lit(as_of_date)) & (F.col("asset_class") == "corporate_bond")
        )
        ...
        write_report(out, cfg, "R02")
        log_step(LOGGER, "report.R02", n_in, out.count(), n_in - filtered.count(), "non_bond_or_other_date", timer.duration_s)
        return out
