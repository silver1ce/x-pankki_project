...
def build(spark: SparkSession, cfg: dict, as_of_date: str) -> DataFrame:
    """How much of the business-loan book finances borrower GHG emissions?"""
    with timed_step(LOGGER, "report.R03") as timer:
        ...
        filtered = fe.filter(
            (F.col("as_of_date") == F.lit(as_of_date)) & (F.col("asset_class") == "business_loan")
        )
        ...
        out = filtered.groupBy("as_of_date", "portfolio_id", "issuer_id", "issuer_name").agg(
            F.round(F.sum("exposure_eur"), 2).alias("outstanding_eur"),
        ...
        write_report(out, cfg, "R03")
        log_step(LOGGER, "report.R03", n_in, out.count(), n_in - filtered.count(), "non_loan_or_other_date", timer.duration_s)
        return out
